...
# NOTE: SCALE_FACTOR above is only the *default* zoom (10); actual samples draw
# a per-sample zoom from GenerationParams.zoom_min/zoom_max (default 9..11), so
# any code converting fine-canvas coords to search-image coords for a specific
# `sample` must use sample["params"]["zoom"], not this module constant.
...
def draw_mat_boundaries(search_img_rgb, mat_rects, zoom, color=(90, 200, 255)):
    """mat_rects are in fine-canvas (1 nm/px) coords; search image pixel size
    is `zoom` nm/px for this specific sample -- scale down by that, not by
    the SCALE_FACTOR default constant, since zoom varies per sample."""
    out = search_img_rgb.copy()
    for (x0, y0, w, h) in mat_rects:
        p0 = (int(round(x0 / zoom)), int(round(y0 / zoom)))
        p1 = (int(round((x0 + w) / zoom)), int(round((y0 + h) / zoom)))
...
for arch, seed in [("dram_1x", 7), ("finfet_10nm", 11)]:
    ...
    with_mats = draw_mat_boundaries(with_gt, sample["mat_rects"], sample["zoom"])
...
# Pinned to a fixed 10:1 zoom (not the default 9..11 random range) so this
# specific illustrative asset demonstrates the aliasing effect at the exact
# ratio the slide text discusses, rather than a random per-run ratio.
params = GenerationParams(zoom_min=10.0, zoom_max=10.0)
...
zoom = sample["zoom"]
search_patch_w, search_patch_h = int(sub_w_ref / zoom), int(sub_h_ref / zoom)
