...
def apply_rotation(img: np.ndarray, angle_deg: float) -> np.ndarray:
    """Rigid in-plane frame rotation about the image center, from sample-stage
    misalignment between the reference and search acquisitions. angle_deg=0
    is a no-op. Border-replicated so rotated corners don't introduce a hard
    black wedge that would bias downstream matching/noise stats.
    """
    if angle_deg == 0.0:
        return img
    h, w = img.shape
...
def downsample_area_average(img: np.ndarray, output_size_px: int) -> np.ndarray:
    """Area-average resize straight to an explicit square output size, rather
    than dividing by an integer factor -- the fine canvas is sized so this
    ratio (search FOV / output size) lands on the caller's target pixel-size
    ratio even when that ratio isn't a whole number (e.g. 9.3:1)."""
    return cv2.resize(img, (output_size_px, output_size_px), interpolation=cv2.INTER_AREA)
...
def image_search(
    ...
    """`output_size_px` is the caller's target Search image size (e.g. 1000).
    The effective pixel-size ratio is implied by full_canvas.shape[0] /
    output_size_px -- the caller controls it by sizing the fine canvas, not by
    passing a ratio here, since the ratio need not be a whole number."""
    blurred = gaussian_psf_blur(full_canvas, spot_size_nm, pixel_size_ref_nm, astigmatism_ratio)
    downsampled = downsample_area_average(blurred, output_size_px)
    ...
    rotated = apply_rotation(distorted, rotation_deg)
    noisy = add_shot_noise(rotated, dose, rng)
