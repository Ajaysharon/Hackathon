...
Orchestrates one Drift-Sense sample:
  ...
  fine canvas (1 nm/px, sized so canvas_px / zoom == 1000)
    -> random 1000x1000 crop = Reference Image (native res)
    -> whole-canvas beam blur + area-average downsample to 1000x1000 +
       search-specific noise/drift = Search Image (1000x1000 @ zoom nm/px)
    -> ground truth = crop location, converted to search-image pixel coords

Physical calibration is fixed by the problem statement for the *image sizes*:
both Reference and Search are always 1000x1000 px, and Reference is always
1 nm/px (1 um FOV). The Reference:Search pixel-size ratio ("zoom") is NOT a
single fixed constant -- it is drawn per sample, uniformly, from
[GenerationParams.zoom_min, zoom_max] (default 9:1 .. 11:1, matching
find_location_fast.py's own FFT zoom estimation range). PIXEL_SIZE_SEARCH_NM
and SCALE_FACTOR below are kept only as the *default/typical* value (10) for
code that wants a representative constant (e.g. display code) -- actual
per-sample generation always uses the drawn zoom, not this constant.
"""
...
PIXEL_SIZE_REF_NM = 1
PIXEL_SIZE_SEARCH_NM = 10  # typical/default value only -- see module docstring
SCALE_FACTOR = PIXEL_SIZE_SEARCH_NM // PIXEL_SIZE_REF_NM  # 10, default only
FINE_CANVAS_SIZE_PX = REFERENCE_SIZE_PX * SCALE_FACTOR  # 10000, default only
...
class GenerationParams:
    ...
    # Max magnitude (degrees) of random in-plane stage-rotation misalignment
    # between reference and search acquisitions. Each sample draws its own
    # angle uniformly from [0, rotation_max_deg]; 0 disables rotation.
    ...
    # Reference:Search pixel-size ratio ("zoom"), drawn per sample uniformly
    # from [zoom_min, zoom_max]. Real SEM tool magnification/dose-vs-FOV
    # tradeoffs mean the operator doesn't hit an exact round-number zoom every
    # time; default 9:1..11:1 brackets the nominal 10:1 the problem statement
    # states, matching find_location_fast.py's own FFT zoom-estimation range
    # (ZOOM_MIN=7, ZOOM_MAX=15) so the baseline matcher isn't over-fit to a
    # single exact ratio. Set zoom_min == zoom_max for the old fixed-ratio
    # behavior.
    zoom_min: float = 9.0
    zoom_max: float = 11.0
...
def _draw_zoom(rng: np.random.Generator, params: GenerationParams) -> float:
    if params.zoom_max <= params.zoom_min:
        return float(params.zoom_min)
    return float(rng.uniform(params.zoom_min, params.zoom_max))
...
def generate_fine_canvas(
    ...
    canvas_size_px: int = FINE_CANVAS_SIZE_PX,
    ...
    return generator(
        canvas_size_px, preset, params.collapse_threshold_nm, rng,
...
def _pick_crop_origin(zone_result: dict, params: GenerationParams, rng: np.random.Generator,
                       canvas_size_px: int = FINE_CANVAS_SIZE_PX) -> tuple:
    max_offset = canvas_size_px - REFERENCE_SIZE_PX
...
def generate_sample(
    ...
    zoom = _draw_zoom(rng, params)
    canvas_size_px = int(round(REFERENCE_SIZE_PX * zoom))

    if preset_overrides:
        fine_canvas = generate_fine_canvas(architecture, rng, params, canvas_size_px, preset_overrides)
        zone_result = {"strip_rects": []}
    else:
        zone_result = generate_fine_canvas_zoned(architecture, rng, params, canvas_size_px)
        fine_canvas = zone_result["canvas"]

    x0, y0 = _pick_crop_origin(zone_result, params, rng, canvas_size_px)
    ...
    reference_img = sem_imaging.image_reference(
        ...
        speckle_sigma=params.speckle_sigma * 0.3,
        salt_pepper_prob=params.salt_pepper_prob * 0.2,
    )

    rotation_deg = float(rng.uniform(0, params.rotation_max_deg)) if params.rotation_max_deg > 0 else 0.0

    search_img = sem_imaging.image_search(
        fine_canvas,
        pixel_size_ref_nm=PIXEL_SIZE_REF_NM,
        output_size_px=REFERENCE_SIZE_PX,
        spot_size_nm=params.beam_spot_size_nm,
    ...
    box_w = box_h = REFERENCE_SIZE_PX / zoom
    gt_x0 = x0 / zoom
    gt_y0 = y0 / zoom
    ...
    return {
        ...
        "params": {**params.as_dict(), "rotation_deg": rotation_deg, "zoom": zoom},
        "rotation_deg": rotation_deg,
        "zoom": zoom,
...
def generate_sample_family(
    ...
    *imaged* differs, which is the point: the same Reference should still be
    ...
    zoom = _draw_zoom(struct_rng, params)
    canvas_size_px = int(round(REFERENCE_SIZE_PX * zoom))
    zone_result = generate_fine_canvas_zoned(architecture, struct_rng, params, canvas_size_px)
    fine_canvas = zone_result["canvas"]
    x0, y0 = _pick_crop_origin(zone_result, params, struct_rng, canvas_size_px)
    crop = fine_canvas[y0:y0 + REFERENCE_SIZE_PX, x0:x0 + REFERENCE_SIZE_PX]

    base_rotation_deg = float(struct_rng.uniform(0, params.rotation_max_deg)) if params.rotation_max_deg > 0 else 0.0
    ...
    for i, overrides in enumerate(variants):
        ...
        search_variants.append({
            "label": label,
            "search_img": search_img,
            "params": {**variant_params.as_dict(), "rotation_deg": base_rotation_deg, "zoom": zoom},
        })
